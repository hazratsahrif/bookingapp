...
void initializeOneSignal([BuildContext? context]) async {
  ...
  if (OneSignal.Notifications.permission) {
    if (appStore.playerId.validate().isEmpty) {
      if (OneSignal.User.pushSubscription.id.validate() !=
          appStore.playerId.validate()) {
    ...
    await OneSignal.Notifications.requestPermission(true).then((value) async {
      if (value) {
        if (appStore.playerId.validate().isEmpty) {
          if (OneSignal.User.pushSubscription.id.validate() !=
              appStore.playerId.validate()) {
  ...
  userService.updatePlayerIdInFirebase(
      ...
      playerId: OneSignal.User.pushSubscription.id.validate());
  print('before observer in user');
  OneSignal.User.pushSubscription.addObserver((changes) async {
    if (!changes.current.id.isEmptyOrNull)
      await setValue(PLAYERID, changes.current.id);
    print('observer of user app${changes.current.id}');
    print('user ---> getStringAsync(PLAYERID) ${getStringAsync(PLAYERID)}');
    updatePlayerId(playerId: changes.current.id.validate());
  });
...
void savePlayerId() async {
  if (appStore.playerId.validate().isEmpty) {
    final osDeviceState = OneSignal.User.pushSubscription.id;
    if (osDeviceState != null) {
      if (osDeviceState.validate() != appStore.playerId.validate()) {
        updatePlayerId(playerId: osDeviceState.validate());
      }
    }
  }
}
  ...
  if (url.validate().isNotEmpty) {
    custom_tabs.launch(
      ...
      customTabsOption: custom_tabs.CustomTabsOption(
        ...
        showPageTitle: true,
        toolbarColor: appStore.isDarkMode ? primaryColorDark : primaryColor,
      ),
...
String formatDate(String? dateTime,
  ...
  final local = parsedDateTime.toLocal();

  return DateFormat(format, languageCode)
      .format(isLocal ? local : parsedDateTime);
}

String timeAgoSinceDate(String? dateString,
    {bool numericDates = true, bool isLanguageNeeded = true}) {
  final languageCode = isLanguageNeeded ? appStore.selectedLanguageCode : null;
  try {
    ...
    if (difference.inDays > 8) {
      ...
      return DateFormat('yyyy/MM/dd', languageCode).format(
        notificationDate,
      );
      ...
      return DateFormat('yyyy/MM/dd', languageCode).format(date); //just now
    }
  } catch (e) {
    return '';
  }
...
void locationWiseServiceForce(BuildContext context, VoidCallback onTap) async {
  Permissions.cameraFilesAndLocationPermissionsGranted().then((value) async {
    await setValue(PERMISSION_STATUS, value);
