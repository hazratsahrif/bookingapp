...
import '../model/banner_ads_model.dart';
...
Future<Map<String, dynamic>?> requestOtp(Map request) async {
  return await handleResponse(await buildHttpResponse('requestOtp',
      request: request, method: HttpMethodType.POST)) as Map<String, dynamic>?;
}

Future<Map<String, dynamic>?> verifyOtp(Map request) async {
  return await handleResponse(await buildHttpResponse('verifyOtp',
      request: request, method: HttpMethodType.POST)) as Map<String, dynamic>?;
}
...
Future<void> saveUserData(UserData data) async {
  ...
  await appStore.setVerifyOTP(data.verify_otp ?? false);
...
Future<void> clearPreferences() async {
  ...
  await appStore.setVerifyOTP(false);
  ...
  if (isAndroid) await OneSignal.Notifications.clearAll();
}
...
Future<void> editUserProfileData(
    {required Map<String, dynamic> req, File? imageFile}) async {
...
Future<DashboardResponse> userDashboard(
  ...
  String endPoint = 'dashboard-detail-new';
...
Future<BannerAdsModel> getBannerAds() async {
  Completer<BannerAdsModel> completer = Completer();
  final endPoint = 'ads-list';
  try {
    final response =
        await buildHttpResponse(endPoint, method: HttpMethodType.GET);
    final bannersResponse =
        BannerAdsModel.fromJson(await handleResponse(response));
    completer.complete(bannersResponse);
  } catch (e) {
    completer.completeError(e);
  }
  return completer.future;
...
void _performAdditionalProcessing(DashboardResponse dashboardResponse) async {
  ...
  if (dashboardResponse.appDownload != null) {
    compareValuesInSharedPreference(
        APPSTORE_URL, dashboardResponse.appDownload!.appstore_url.validate());
    compareValuesInSharedPreference(PLAY_STORE_URL,
        dashboardResponse.appDownload!.playstore_url.validate());
    compareValuesInSharedPreference(PROVIDER_PLAY_STORE_URL,
        dashboardResponse.appDownload!.provider_playstore_url.validate());
    compareValuesInSharedPreference(PROVIDER_APPSTORE_URL,
        dashboardResponse.appDownload!.provider_appstore_url.validate());
  }
  ...
  if (dashboardResponse.generalSetting != null) {
    ...
    compareValuesInSharedPreference(
        SITE_DESCRIPTION, generalSetting.siteDescription.validate());
    compareValuesInSharedPreference(
        SITE_COPYRIGHT, generalSetting.siteCopyright.validate());
    compareValuesInSharedPreference(
        FACEBOOK_URL, generalSetting.facebookUrl.validate());
    compareValuesInSharedPreference(
        INSTAGRAM_URL, generalSetting.instagramUrl.validate());
    compareValuesInSharedPreference(
        TWITTER_URL, generalSetting.twitterUrl.validate());
    compareValuesInSharedPreference(
        LINKEDIN_URL, generalSetting.linkedinUrl.validate());
    compareValuesInSharedPreference(
        YOUTUBE_URL, generalSetting.youtubeUrl.validate());

    appStore.setInquiryEmail(
        generalSetting.inquriyEmail.validate(value: INQUIRY_SUPPORT_EMAIL));
    appStore.setHelplineNumber(
        generalSetting.helplineNumber.validate(value: HELP_LINE_NUMBER));
  }

  compareValuesInSharedPreference(IS_ADVANCE_PAYMENT_ALLOWED,
      dashboardResponse.isAdvancedPaymentAllowed == '1');
  ...
  if (appStore.isLoggedIn) {
    await getAppConfigurations().catchError(onError);
  }
}
...
Future<ServiceDetailResponse> getServiceDetails(
  ...
  if (!listOfCachedData.any((element) => element?.$1 == serviceId)) {
    listOfCachedData.add((serviceId, res));
  } else {
    int index =
        listOfCachedData.indexWhere((element) => element?.$1 == serviceId);
    listOfCachedData[index] = (serviceId, res);
  }
