...
class _PendingBookingComponentState extends State<PendingBookingComponent> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.upcomingData!.first.status != BOOKING_STATUS_PENDING &&
        widget.upcomingData!.first.status != BOOKING_STATUS_ACCEPT) {
      return Offstage();
    }
    ...
    return Container(
      ...
      child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                          formatDate(
                              widget.upcomingData.validate().first.date ??
                                  DateTime.now().toString(),
                              format: widget.upcomingData
                                          .validate()
                                          .first
                                          .bookingSlot !=
                                      null
                      ...
                      Text(
                          " ${widget.upcomingData.validate().first.bookingSlot ?? ''}",
                          style:
                              primaryTextStyle(color: Colors.white, size: 14)),
                    ],
                  ),
        ...
        BookingDetailScreen(bookingId: widget.upcomingData.validate().first.id!)
            .launch(context);
      }),
    );
  }
